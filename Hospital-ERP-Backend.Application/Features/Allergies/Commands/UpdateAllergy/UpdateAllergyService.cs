...
namespace Hospital_ERP_Backend.Application.Features.Allergies.Commands.UpdateAllergy
{
        ...
        IRequestHandler<UpdateAllergyRequest, UpdateAllergyResponse>
    {
        ...
        private async Task<UpdateAllergyResponse> UpdateAllergyAsync(UpdateAllergyRequest request)
        {
            ...
            Allergy? allergy = await _repository.FindAsync(request.Id);
            ...
            if (result == null)
            {
                throw new InvalidOperationException("Failed to update Allergy.");
            }
